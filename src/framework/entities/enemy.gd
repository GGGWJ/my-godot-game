	...
	if player != null:
		...
		if pathfinding != null:
			...
			if position.distance_squared_to(player.global_position) > stop_dist * stop_dist:
				...
				if mover_component: mover_component.stop()
			...
			var stop_dist = enemy_stats.stop_distance if enemy_stats else 10.0
			if position.distance_squared_to(player.global_position) > stop_dist * stop_dist:
				if mover_component: mover_component.move_in_direction(direction)
			else:
				if mover_component: mover_component.stop()
...
func _on_navigation_velocity_computed(safe_velocity: Vector2) -> void:
	...
	if mover_component:
		mover_component.apply_velocity(safe_velocity)
